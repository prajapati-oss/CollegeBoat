...
#         # Create vector store
#         # query_name must match your Supabase function: match_documents(query_embedding, match_threshold, match_count)
#         vectorstore = SupabaseVectorStore(
#             client=supabase_client,
#             embedding=embedding,
#             table_name="college_vectors",
...
#             chunk_size=500,
#         )
#         #vectorstore.match_threshold = 0.4
...
#             # Track year matches for same course/semester/subject
#             if semester and course and paper_subject:
#                 if fuzz.partial_ratio(subject, paper_subject) > 70:
#                     year_matches.append({
#                         'paper': paper,
...
#         # If user specified a year and we have multiple subject matches, prefer the exact year
...
#         if st.button("Uploadvector"):
#             if not vectors:
#                 st.error("No vectors found. Please upload a file first.")

#             # supabase.table("college_vectors").insert(vectors).execute()
#             # st.success("Data stored in Supabase vector database")

#             else:
#                 response = supabase.table("college_vectors").insert(vectors).execute()

#                 print(response)

#                 st.success(f"{len(st.session_state.vectors)} vectors stored successfully")
...
#                    url=paper.get('file_url')
#                    if url:
#                        # Normalize S3 URL to correct region (ap-south-1)
#                        import re as _re
#                        url = _re.sub(
...
# -----------------------
# FIX (Bug #1): Supabase client must be created ONCE per app lifetime,
# not on every Streamlit rerun. Previously the client was created twice,
# unconditionally, at module import time (which runs on EVERY rerun in
# Streamlit). Each call to create_client() spins up a new httpx.Client
# with its own connection pool that is never closed, eventually
# exhausting sockets/file descriptors and causing httpx.ConnectError.
#
# @st.cache_resource ensures create_client() only runs once and the
# same client/connection pool is reused across reruns and sessions.
# -----------------------
@st.cache_resource
def init_supabase():
    try:
...
embedding_model = HuggingFaceEmbeddings(
    model_name="sentence-transformers/all-MiniLM-L6-v2",model_kwargs={"device": "cpu"}, encode_kwargs={"normalize_embeddings": True})
    ...
    def uploadfile():
        ...
        if st.button("Upload"):
            if uploaded_file and subject:
                ...
                # Insert metadata into PostgreSQL (Supabase)
                try:
                    supabase.table("question_papers").insert({
                        "subject": subject,
                        "semester": semester,
        ...
        if st.button("Uploadvector"):
            if not vectors:
                ...
                try:
                    ...
                    st.error(f"Failed to store vectors: {e}")
...
""", unsafe_allow_html=True)
    ...
    # -----------------------
    # FIX (Bug #2): save_chat previously had no guard against a missing
    # user_id (anonymous users) and no error handling around the network
    # call. If user_id was None, the insert could fail (FK/not-null
    # violation) or, once the Supabase client's connection pool was
    # exhausted from being re-created every rerun (Bug #1), the request
    # could fail with httpx.ConnectError. Since nothing caught the
    # exception, it crashed the entire app. Now we skip saving for
    # anonymous users and always wrap the network call in try/except so
    # a transient failure just shows a warning instead of crashing.
    # -----------------------
    def save_chat(user_id, role, message):
        if not user_id:
            # Anonymous / not logged in — don't attempt to save, and
